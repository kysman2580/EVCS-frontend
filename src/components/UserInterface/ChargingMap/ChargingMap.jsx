...
import customMarkerSrc from '../public/image/2.png';
...
const KakaoMap = () => {
  ...
  useEffect(() => {
    ...
    script.async = true;
    const kakaoServiceKey =
      "서비스키"
    script.src =
      `https://dapi.kakao.com/v2/maps/sdk.js?appkey=${kakaoServiceKey}&autoload=false&libraries=services`;
    ...
    script.onload = () => {
      window.kakao.maps.load(() => {
        ...
        const customMarkerImage = new window.kakao.maps.MarkerImage(customMarkerSrc, customImageSize);
        ...
        function fetchEVChargerInfo(apiZcode, userDetailedZscode, numOfRows = 9999) {
          const serviceKey =
            "서비스키";
              ...
              if (items.length > 0) {
                for (let i = 0; i < items.length; i++){
                  ...
                  if (latNode && lngNode && zscodeNode) {
                    ...
                    if (userDetailedZscode && stationZscode !== userDetailedZscode) continue;
                    ...
                    const chgerTypeValue = chgerTypeNode ? chgerTypeNode.textContent : "";
                    const chgerTypeText = chgerTypeMapping[chgerTypeValue] || chgerTypeValue;
            ...
            (position) => {
              ...
              geocoder.coord2RegionCode(userLng, userLat, function(result, status){
                if (status === window.kakao.maps.services.Status.OK && result.length > 0) {
                  ...
                  console.log("provinceName:", provinceName, "districtName:", districtName);
                  
                  const apiZcode = provinceMapping[provinceName] || "11";
                  const provinceDetail = detailedMapping[provinceName];
                  const userDetailedZscode = provinceDetail ? provinceDetail[districtName] : null;
                  console.log("API zcode:", apiZcode, "User detailed zscode:", userDetailedZscode);
